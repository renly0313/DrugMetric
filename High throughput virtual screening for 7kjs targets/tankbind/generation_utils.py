...
def distance_optimize_compound_coords(coords, y_pred, protein_nodes_xyz, 
                        compound_pair_dis_constraint, total_epoch=5000, loss_function=distance_loss_function, LAS_distance_constraint_mask=None, mode=0, show_progress=False, device=torch.device('cpu')):
    # random initialization. center at the protein center.
    c_pred = protein_nodes_xyz.mean(axis=0)
    x = (5 * (2 * torch.rand(coords.shape, device=device) - 1) + c_pred.reshape(1, 3).detach())
...
def get_info_pred_distance(coords, y_pred, protein_nodes_xyz, compound_pair_dis_constraint, n_repeat=10, LAS_distance_constraint_mask=None, mode=0, show_progress=False, device=torch.device('cpu')):
    info = []
    if show_progress:
    ...
    for repeat in it:
        ...
        x, loss_list, rmsd_list = distance_optimize_compound_coords(coords, y_pred, protein_nodes_xyz, compound_pair_dis_constraint, LAS_distance_constraint_mask=LAS_distance_constraint_mask, mode=mode, show_progress=False, device=device)
