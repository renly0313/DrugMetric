...
def GRU(x, h_nei, W_z, W_r, U_r, W_h):
    ...
    sum_h = h_nei.sum(dim=1)#[batch_size, hidden_size]
    z_input = torch.cat([x, sum_h], dim=1)#[batch_size, hidden_size + hiddensize]
    #z = F.sigmoid(W_z(z_input))
    z = torch.sigmoid(W_z(z_input))

    r_1 = W_r(x).view(-1, 1, hidden_size)#[batch_size, hidden_size]->[batch_size, 1, hidden_size]
    r_2 = U_r(h_nei)#[batch_size, MAX_NB, hidden_size]
    #r = F.sigmoid(r_1 + r_2)
    r = torch.sigmoid(r_1 + r_2)#[batch_size, MAX_NB, hidden_size]
    
    gated_h = r * h_nei#[batch_size, MAX_NB, hidden_size]
    sum_gated_h = gated_h.sum(dim=1)#[batch_size, hidden_size]
    h_input = torch.cat([x, sum_gated_h], dim=1)#[batch_size, hidden_size + hidden_size]
    #pre_h = F.tanh(W_h(h_input))
    pre_h = torch.tanh(W_h(h_input))#[batch_size, hidden_size]
    new_h = (1.0 - z) * sum_h + z * pre_h#[batch_size, MAX_NB, hidden_size]
    return new_h
