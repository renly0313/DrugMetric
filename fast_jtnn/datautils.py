    ...
    def __iter__(self):
        for fn in self.data_files:
            ...
            dataloader = DataLoader(dataset, batch_size=1, shuffle=False,
                                    collate_fn=lambda x: x[0])  # , num_workers=self.num_workers)
...
def set_batch_nodeID(mol_batch, vocab):
    ...
    for mol_tree in mol_batch:
        for node in mol_tree.nodes:
            node.idx = tot
            node.wid = vocab.get_index(
                node.smiles)  # Get index of the corresponding cluster vocabulary item. 获取相应的集群词汇项的索引。
            tot += 1
...
def get_data_processed(data_path, args):
    ...
    data_path = os.path.join(data_path,
                             'mol_tree_mean={}_mol_tree_std={}'.format(args.mol_tree_mean, args.mol_tree_std))
