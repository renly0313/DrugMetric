...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG):
        ...
        :param depthG: int
            The number of timesteps for implementing message passng for encoding the molecular graphs.
        """
        super(JTNNVAE, self).__init__()
        self.vocab = vocab
        self.hidden_size = hidden_size
        # size of latent space (latent representation involves both tree and graph encoding vertors)
        # 潜空间的大小（潜伏表示涉及树和图的编码顶点）。
        ...
        # for encoding for candidate subgraphs, in the graph decoding phase (section 2.5)
        # 用于候选子图的编码，在图解码阶段（2.5节）。
        self.jtmpn = JTMPN(hidden_size, depthG)
        # encoder for producing the molecule graph encoding given batch of molecules
        # 编码器，用于生成分子图，对给定的一批分子进行编码
    ...
    def encode(self, jtenc_holder, mpn_holder):
        tree_vecs, tree_mess = self.jtnn(*jtenc_holder)#tree_vecs:[batch_size, hidden_size] tree_mess：[num_edges, hidden_size]
        mol_vecs = self.mpn(*mpn_holder)#mol_vecs: [batch_size, hidden_size]
        return tree_vecs, tree_mess, mol_vecs

    def re_encode(self, mol_batch):
        tree_batch = [MolTree(s) for s in mol_batch]
    ...
    def rsample(self, z_vecs, W_mean, W_var):
        ...
        z_mean = W_mean(z_vecs)#[batch_size, hidden_size]->[batch_size, latent_size]
        z_log_var = -torch.abs(W_var(z_vecs))#[batch_size, hidden_size]->[batch_size, latent_size] #Following Mueller et al.

        # as per Kingma & Welling
        kl_loss = -0.5 * torch.sum(1.0 + z_log_var - z_mean * z_mean - torch.exp(z_log_var)) / batch_size

        # reparameterization trick
        epsilon = create_var(torch.randn_like(z_mean))#[batch_size, latent_size]里面的元素服从均值为0， 方差为1的标准正态分布
    ...
    def forward(self, x_batch, beta):
        ...
        tree_mean = self.T_mean(x_tree_vecs)
        mol_mean = self.G_mean(x_mol_vecs)
        tree_var = -torch.abs(self.T_var(x_tree_vecs))
        mol_var = -torch.abs(self.G_var(x_mol_vecs))
        mol_tree_mean = torch.mean(torch.cat([tree_mean, mol_mean], dim=1), dim=0)
        mol_tree_var = torch.mean(torch.cat([tree_var, mol_var], dim=1),dim=0)

        mol_tree_std = mol_tree_var.mul(0.5).exp_()
        return word_loss + topo_loss + assm_loss + beta * kl_div, kl_div.item(), word_acc, topo_acc, assm_acc, mol_tree_mean, mol_tree_std


    def forward_dis(self, x_batch):
        x_batch, x_jtenc_holder, x_mpn_holder, x_jtmpn_holder = x_batch#数据处理
        x_tree_vecs, x_tree_mess, x_mol_vecs = self.encode(x_jtenc_holder, x_mpn_holder)#编码得到vec
        tree_mean = self.T_mean(x_tree_vecs)
        mol_mean = self.G_mean(x_mol_vecs)
        # tree_var = -torch.abs(self.T_var(x_tree_vecs))
        # mol_var = -torch.abs(self.G_var(x_mol_vecs))
        mol_tree_mean = torch.cat([tree_mean, mol_mean], dim=1)
        # mol_tree_logvar = torch.mean(torch.cat([tree_var, mol_var], dim=1),dim=0)
        # mol_tree_var = mol_tree_logvar.exp_()
        # return mol_tree_mean, mol_tree_var

        return mol_tree_mean
    ...
    def reconstruct(self, smiles, prob_decode=False):
        _, tree_vec, mol_vec = self.re_encode([smiles])
        ...
        epsilon = create_var(torch.randn(1, int(self.latent_size)), False)
